...
def choose_by_risk(primary_pool: pd.DataFrame, fallback_pool: pd.DataFrame, specs: list[tuple[str, int]]) -> pd.DataFrame:
    selected = []
...
single_sample = choose_by_risk(
    primary_single_pool,
    base_single_pool,
    [("Low", 4), ("Medium", 3), ("High", 3)],
)
basket_sample = choose_by_risk(
    primary_basket_pool,
    base_basket_pool,
...
print(f"demo_catalog.json: {len(catalog)} items; {catalog_wins} curated model-win examples")

# ── demo_orders.json ─────────────────────────────────────────────────────────
valid_orders = demo_df[(demo_df[TARGET_COL] > 0) & demo_df["customer_state"].notna()].copy()

if has_model_predictions:
    priority_orders = valid_orders[
        valid_orders["model_better_than_olist"] | valid_orders["model_prevents_late_surprise"]
    ].copy()
    if len(priority_orders) >= TARGET_ROWS:
        order_pool = priority_orders
        print(f"Sampling demo_orders from {len(priority_orders):,} validation/test rows where model beats or de-risks Olist.")
    else:
        # Include every strong win and fill from the same validation/test period.
        fill_pool = valid_orders[~valid_orders[ORDER_ID_COL].isin(priority_orders[ORDER_ID_COL])]
        needed = TARGET_ROWS - len(priority_orders)
        order_pool = pd.concat([priority_orders, stratified_sample(fill_pool, needed)], ignore_index=True)
        print(f"Only {len(priority_orders):,} strong model-win rows found; filling with validation/test rows.")
else:
    order_pool = valid_orders
...
demo_orders = stratified_sample(order_pool, TARGET_ROWS)
demo_orders = demo_orders[order_cols].copy()
