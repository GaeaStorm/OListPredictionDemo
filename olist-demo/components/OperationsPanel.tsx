...
export default function OperationsPanel({
  ...
  const showHistoricalOnly = historicalDays != null && !showComparison

  const olistDate = purchaseDate && historicalDaysEstimated != null
    ? computeDate(purchaseDate, historicalDaysEstimated)
    : null
  const actualDate = historicalDeliveryDate ?? (purchaseDate && historicalDays != null
    ? computeDate(purchaseDate, historicalDays)
    : null)

  const olistError = historicalDays != null && historicalDaysEstimated != null
    ? historicalDaysEstimated - historicalDays
    : null
    ...
    <div className="rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden shadow-sm">
      ...
      {open && (
        <div className="border-t border-slate-200 px-5 pb-5 pt-4 space-y-5">
          ...
          {/* Delivery comparison for the selected historical order */}
          {(showComparison || showHistoricalOnly) && (
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">
                Delivery Comparison
              </h4>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    ...
                    <div className="rounded-lg bg-slate-50 border border-slate-200 p-2">
                      ...
                      {olistDate && <p className="text-[10px] text-slate-400">{olistDate}</p>}
                      {olistError != null && (
                    ...
                    <div className="rounded-lg bg-white border border-slate-300 p-2">
                      <p className="text-[10px] text-slate-500 font-semibold uppercase">Actual</p>
                      <p className="text-lg font-bold text-slate-800">{historicalDays!.toFixed(1)}d</p>
                      {actualDate && <p className="text-[10px] text-slate-400">{actualDate}</p>}
                      <p className="text-[10px] text-slate-400 mt-0.5">from dataset</p>
                    </div>
                  </div>
                ...
                <div className="rounded-xl bg-white border border-slate-200 p-3 space-y-2">
                  <div className="flex items-start gap-4">
                    <div className="shrink-0">
                      <p className="text-xs text-slate-500 font-medium">Dataset actual</p>
                      <p className="text-xl font-bold text-slate-800">{historicalDays!.toFixed(1)}</p>
                      <p className="text-xs text-slate-400">days</p>
                      {actualDate && <p className="text-xs text-slate-400 mt-0.5">{actualDate}</p>}
                    </div>
